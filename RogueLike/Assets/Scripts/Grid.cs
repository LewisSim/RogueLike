...
public class Grid : MonoBehaviour
{
    ...
    public Transform StartPosition;//where the program will start the pathfinding from.
    public LayerMask WallMask;//the mask that the program will look for when trying to find obstructions to the path.
    public Vector2 vGridWorldSize;//vector2 to store the width and height of the graph in world units.
    public float fNodeRadius;// stores how big each square on the graph will be
    public float fDistanceBetweenNodes;

    Node[,] NodeArray;// array of nodes that the algorithm uses.
    public List<Node> FinalPath;//completed path that the red line will be drawn along
